...
fn apply_bc(
    ...
    match bc {
        ...
        BoundaryCondition::Convective { h, t_fluid } => {
            ...
            //
            // The wall cell temperature is stored at the cell centroid. The
            // centroid-to-surface half-cell conduction resistance must be
            // included in series with the convective film resistance:
            //
            //   1/K_eff = 1/(h*A) + 1/face_conductance
            //
            // Contribution to cell: K_eff * (T_fluid - T_centroid)
            let h_a = h * wall_area;
            if h_a <= 0.0 {
                return;
            }
