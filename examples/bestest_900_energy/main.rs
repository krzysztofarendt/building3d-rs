...
// line for "900 - Heavyweight Base Case" (OpenStudio/EnergyPlus).
//
...
fn bestest_900_constructions() -> (
    ...
    // Heavy-mass case uses HWWALL + HWFLOOR, while roof and window match Case 600.
    let (_lt_wall, lt_roof, _lt_floor, window) = bestest_600_like_roof_window_constructions();
    ...
    // In BESTEST-GSR resources, R-25 is 25.175 m2*K/W.
    let r25 = 25.175_f64;
...
fn bestest_600_like_roof_window_constructions() -> (
    WallConstruction,
    WallConstruction,
    WallConstruction,
    WallConstruction,
) {
    // Local helper to preserve Case 600 roof/window definitions used by Case 900.
...
/// Builds the BESTEST single-zone base geometry used by Case 900.
fn build_bestest_900() -> Result<Building> {
    let x = 8.0;
    let y = 6.0;
    let z = 2.7;
    ...
    Building::new("bestest_900", vec![zone])
}

fn suite_case_900_config(building: &Building) -> ThermalConfig {
    let (hw_wall, lt_roof, hw_floor, window) = bestest_900_constructions();
    ...
    cfg.constructions.insert("floor".to_string(), hw_floor);
    cfg.constructions.insert("wall".to_string(), hw_wall);
...
fn case_900_solar_config() -> SolarGainConfig {
...
fn sweep_cases() -> Vec<SweepCase> {
    vec![
        SweepCase {
            ...
            description: "bestest_energy_suite Case 900 modeling style",
        ...
        SweepCase {
            id: "suite_no_warmup",
            label: "Suite + no warmup",
            description: "solver control: 0 warmup days",
            kind: SweepKind::SuiteNoWarmup,
        },
        SweepCase {
            id: "suite_substeps_1",
            label: "Suite + 1 substep",
            description: "solver control: 1 substep per hour",
            kind: SweepKind::SuiteSubsteps1,
        },
    ]
}
...
fn configure_case(
    ...
    match case.kind {
        ...
        SweepKind::SuiteViewFactorRadiation => {
            ...
            cfg.view_factor_rays_per_surface = std::env::var("BESTEST_900_VF_RAYS")
                .ok()
                .or_else(|| std::env::var("BESTEST_VF_RAYS").ok())
        ...
        SweepKind::SuiteNoWarmup => {
            options.warmup_hours = 0;
        }
        SweepKind::SuiteSubsteps1 => {
...
fn case_filter(all_cases: &[SweepCase]) -> Result<Vec<SweepCase>> {
    let filter_raw = std::env::var("BESTEST_900_SWEEP_CASES")
        .ok()
        .or_else(|| std::env::var("BESTEST_SWEEP_CASES").ok());
    let Some(filter_raw) = filter_raw else {
    ...
    if selected.is_empty() {
        return Err(anyhow!(
            "BESTEST_900_SWEEP_CASES/BESTEST_SWEEP_CASES matched no cases. Available ids: {}",
            all_cases
                .iter()
                .map(|c| c.id)
                .collect::<Vec<_>>()
...
fn write_feature_sweep_barplot_svg(path: &Path, rows: &[SweepResult]) -> Result<()> {
    ...
    writeln!(
        f,
        "<text x='{}' y='44' font-family='monospace' font-size='30' fill='#111827'>BESTEST 900 Feature Sweep</text>",
        left_margin
    )?;
    writeln!(
        f,
...
fn run_sweep_case(
    ...
    let mut cfg = suite_case_900_config(building);
    let mut solar = case_900_solar_config();
    let mut options = base_options();

    configure_case(case, &mut cfg, &mut solar, &mut options);
...
fn main() -> Result<()> {
    let building = build_bestest_900()?;

    let epw_path = std::env::var("BESTEST_900_EPW")
        .ok()
        .or_else(|| std::env::var("BESTEST_600_EPW").ok())
        .map(PathBuf::from)
        .unwrap_or_else(default_epw_path);
    ...
    println!("BESTEST 900 feature sweep (building3d vs OpenStudio/E+ reference)");
    println!(
        "Weather: {} ({} hours)",
        weather.location,
        weather.num_hours()
    ...
    // Keep monthly CSV behavior for quick continuity checks, from suite baseline.
    let baseline_monthly_path = repo_root().join("examples/bestest_900_energy/results.csv");
    write_monthly_results_csv(&baseline_monthly_path, &rows[suite_ref_idx].annual)?;

    let sweep_csv_path = repo_root().join("examples/bestest_900_energy/feature_sweep.csv");
    write_feature_sweep_csv(&sweep_csv_path, &rows, suite_ref_idx)?;

    let sweep_plot_path = repo_root().join("examples/bestest_900_energy/feature_sweep_barplot.svg");
